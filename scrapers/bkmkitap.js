...
const getBooks = async (scrapingSize = SCRAPING_SIZE) => {
    ...
    for (
        let i = 0;
        i < Math.floor(categoriesURLArray.length / FETCH_PER_PAGE);
        i++
    ) {
                    ...
                    async (
                        ...
                        const scrapeProducts = async (htmlText) => {
                            ...
                            div.querySelectorAll(
                                '.showcaseWrap div:not(.catalogWrapper) .row .swiper-wrapper > .productItem'
                            ).forEach((node) => {
                                // Selectorlere bir daha bak! daha düzgün ilerle
                                let regexForPrice =
                                    /[1-9][0-9]*(?:\/[1-9][0-9])*/g
                                let productRatingWidth =
                                    node.querySelector(
                                        '.row:nth-child(2) .stars'
                                    ).style.width || '0%'
                                productRatingWidth =
                                    productRatingWidth === '0%'
                                        ? 0
                                        : Math.round(
                                              parseFloat(productRatingWidth) /
                                                  20
                                          )

                                let obj = {
                                    img:
                                        node.querySelector('.row a img').src ||
                                        null,
                                    rating: productRatingWidth,
                                    URL:
                                        node.querySelector('.row a').href || '',
                                    title:
                                        node.querySelector(
                                            '.row:nth-child(2) .row > .box > .row a:first-child'
                                        ).title || '',
                                    author:
                                        node.querySelector(
                                            '.row:nth-child(2) .row > .box > .row a:nth-child(3)'
                                        ).text || '',
                                    publisher:
                                        node.querySelector(
                                            '.row:nth-child(2) .row > .box > .row a:nth-child(2)'
                                        ).text || '',
                                    price:
                                        parseFloat(
                                            node
                                                .querySelector(
                                                    '.row:nth-child(2) .row .productPrice .currentPrice'
                                                )
                                                .textContent.match(
                                                    regexForPrice
                                                )
                        ...
                        const fetchAPI = async (counter, categoryIndex) => {
                            console.log(
                                `Fetching ${counter} of ${categoriesURLArray[categoryIndex].URL}`
                            )
