...
const getCategories = async () => {
    ...
    const categoryArray = await page.evaluate(async () => {
        ...
        div.innerHTML = await fetch("https://www.kitapyurdu.com/index.php?route=product/category", {
                "headers": {
        ...
        div.querySelectorAll('#content > .grid_5 > a').forEach((node) => {
            ...
            categoryIdArray.push(node.href.split('/').slice(-1)[0].split('.')[0]);
        ...
        return page
            ...
            .then(async () => {
                const outputArray = await page.evaluate(
                    async (index, scrapingSize, parsedArray, categoriesIDs, PAGE_LIMIT) => {
                            let productsArray = [];
                            ...
                            const scrapeProducts = async (htmlText) => {
                                ...
                                if (div.querySelector('.product-grid').children.length < PAGE_LIMIT) paginationFlag = 0;

                                return tempArray;
                            };

                            const fetchAPI = async (counter, categoryID) => {
                                console.log(`Fetching ${counter} of Category-${categoryID}`);

                                return fetch(`https://www.kitapyurdu.com/index.php?route=product/category/&filter_category_all=true&category_id=${categoriesIDs[categoryID]}&sort=purchased_365&order=DESC&filter_in_stock=1&page=${counter}&limit=${PAGE_LIMIT}`, {
                                        "headers": {
                                            "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
                                            "accept-language": "en-US,en;q=0.9,tr;q=0.8",
